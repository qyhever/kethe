...
REMOTE_DIR="${REMOTE_DIR:-/opt/apps/kethe-backend}"
IMAGE_NAME="${IMAGE_NAME:-kethe-backend}"
...
"${COMPOSE_COMMAND[@]}" exec -T app node -e \
  "fetch('http://127.0.0.1:' + (process.env.PORT || 6308) + '/api/meta').then((response) => { if (!response.ok) throw new Error('HTTP ' + response.status); return response.text(); }).then((body) => console.log('服务检查通过：' + body))"
